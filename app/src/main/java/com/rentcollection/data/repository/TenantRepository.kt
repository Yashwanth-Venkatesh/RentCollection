...
class TenantRepository @Inject constructor(
    ...
    fun getAllActiveTenantsFlow(): Flow<List<Tenant>> = callbackFlow {
        val listener = firestore.collection("tenants")
            .whereEqualTo("isActive", true)
            .addSnapshotListener { snap, err ->
                if (err != null) { close(err); return@addSnapshotListener }
                val tenants = snap?.documents?.mapNotNull { it.toObject(Tenant::class.java) }
    ...
    fun getActiveTenantForHouseFlow(houseId: String): Flow<Tenant?> = callbackFlow {
        val listener = firestore.collection("tenants")
            .whereEqualTo("houseId", houseId)
            .addSnapshotListener { snap, err ->
                if (err != null) { close(err); return@addSnapshotListener }
                val active = snap?.documents
                    ?.mapNotNull { it.toObject(Tenant::class.java) }
